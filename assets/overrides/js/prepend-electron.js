...
// Self-invocation fallback when loaded via NODE_OPTIONS
if (process.env.HTTP_TOOLKIT_ACTIVE === 'true') {
    try {
        const fs = require('fs');
        const certPath = process.env.NODE_EXTRA_CA_CERTS;
        if (certPath && fs.existsSync(certPath)) {
            const certData = fs.readFileSync(certPath, 'utf8');
            const spki = process.env.HTTP_TOOLKIT_SPKI || '';
            module.exports({
                newlineEncodedCertData: certData,
                spkiFingerprint: spki
            });
        }
    } catch (e) {
        console.error('Failed to self-initialize prepend-electron', e);
    }
}